Release Notes for Pyrrha 1.0.1
======================================================

Pyrrha version 1.0.1 is now available from:
...
- pyrrha cpu miner improvements

Commit details
--------------

- `3f36ee152` Add pyrrha 1.0.0 release notes (Andrea Suisani)
- `fbd1b29a8` Update release notes template (Andrea Suisani)
- `032da8cb2` [qa] Change constants and env vars name to reflect we are on the pyrrha chain (Andrea Suisani)
...
- `714526782` Fix rpc_getblockstats.py by adding correct genesis hashes for pyrrha (ptschip)
- `5694354bd` qa: Adapt electrum tests to Pyrrha (Dagur Valberg Johannsson)
...
- `db91151dc` electrum: Change default ports for Pyrrha. (Dagur Valberg Johannsson)
...
- `1bb8ce677` Produce libpyrrha while cross-compiling (Andrea Suisani)
...
- `1f6c98de5` Changes to support BCH (and BCH testnet and regtest). All changes are cashlib or BCH signing only. Rename and add java API functions to reflect Pyrrha vs Bch. Put basic chainparams for bch testnet and regtest so address conversion works. (Andrew Stone)
...
- `b9ec47b3c` Prevent the pyrrha-miner from exiting after any excpetions (Peter Tschipper)
...
- `8add0bc5c` Translation file name changes from bitcoin/BCH to Pyrrha (ptschip)
