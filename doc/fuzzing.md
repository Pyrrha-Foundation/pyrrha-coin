...
A special test harness `test_pyrrha_fuzzy` is provided to provide an easy
...
enable LLVM persistent mode, `test_pyrrha_fuzzy` has to be built with
...
```
$AFLPATH/afl-fuzz -i ${AFLIN} -o ${AFLOUT} -m 200 -- test/test_pyrrha_fuzzy
```

And to fuzz using just a single fuzzing entrypoint, specify a
corresponding command line argument to test_pyrrha_fuzzy on what to
fuzz. A list of all possible arguments can be printed like this:

```
test/test_pyrrha_fuzzy list_tests
```

So, for example, to fuzz the CBlock de-/serialization, you can use this:
```
$AFLPATH/afl-fuzz -i ${AFLIN} -o ${AFLOUT} -m 200 -- test/test_pyrrha_fuzzy cblock_deser
```
...
`test_pyrrha_fuzzy`. This will just call the instrumented program and
...
The code has been updated to be more easily extensible. In test_pyrrha_fuzzy.cpp, 
