...
UniValue importprivkey(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"pyrrhaprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"pyrrhaprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importprivatekeys(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1)
        throw runtime_error(
            "importprivatekeys [rescan | no-rescan] \"pyrrhaprivatekey\"...\n"
            ...
            "2. \"pyrrhaprivatekey\"   (string, at least 1 required) The private keys (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue &params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Pyrrha address or script");
    }
...
UniValue dumpprivkey(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("dumpprivkey \"pyrrhaaddress\"\n"
                            "\nReveals the private key corresponding to 'pyrrhaaddress'.\n"
                            "Then the importprivkey can be used with this output\n"
                            "\nArguments:\n"
                            "1. \"pyrrhaaddress\"   (string, required) The pyrrha address for the private key\n"
                            "\nResult:\n"
    ...
    if (!IsValidDestination(dest))
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Pyrrha address");
    }
    CKey vchSecret;
...
UniValue dumpwallet(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "existing files.\n"
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to pyrrhad)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Pyrrha %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
