...
#
# 1. CAUTION!!!! This kills any running pyrrhad's as part of cleanup!
...
# optional tool for use with "test_pyrrha" tests
...
# sequentially.
PARALLEL_TEST_TOOL=gtest-parallel-pyrrha
...
TEST_COMMAND=${3:-"src/test/test_pyrrha"}
...
# helper function to check if parameter is some form of test_pyrrha
# this function has been dumbed down for now.
is_test_pyrrha()
{
    if [ "$1" = "src/test/test_pyrrha" ]
    then
        return 0
    else
        return 1
...
do
    ...
    # check if the test is src/test/test_pyrrha
    is_test_pyrrha "${TEST_COMMAND}"
    if [ $? -eq 0 ]
    then
